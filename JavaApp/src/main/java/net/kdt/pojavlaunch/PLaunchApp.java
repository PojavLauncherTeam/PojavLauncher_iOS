            ...
            try {
                ...
                if (AccountJNI.CURRENT_ACCOUNT.accessToken.equals("0")) {
                    if (!new File(verJsonDir).exists()) {
                        ...
                        UIKit.showError("Error", "Minecraft couldn't be legally installed when logged in with a local account. Please switch to an online account to continue.", false);
                        return;
                    } else {
